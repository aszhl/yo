...
model = YOLO(r"F:\yolov8\ultralytics-main\ultralytics-main\best.pt")  # load a pretrained model (recommended for training)

# Train the model
#results = model.train(data=r"breathe_data/data.yaml",epochs=100)
#print(time.time(),'模型加载完毕')
results = model.predict(source = r"C:\Users\78771\Desktop\微信图片_20240715183527.jpg",save=True)
#print(results)
#pred =time.time()
